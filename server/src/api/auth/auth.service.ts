...
import { seedTerritories } from "../../seeding/territory.seed";
import { seedOptions } from "../../seeding/options.seed";
...
export const seedDatabase = async () => {
  prisma.$transaction(async (tx) => {
    try {
      await seedTerritories(tx as any);
      await seedOptions(tx as any);
    } catch (error) {
      logger.error(error);
    }
  });
...
export const getCurrentUserContext = async (userId: string, clientId: number) => {
