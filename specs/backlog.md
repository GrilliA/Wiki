# Backlog

- As as less tables as possible in the backend, the structure should be dictated by the backend code
- Make it possible for user to change language (with i8n)
- Make The view page first always

## General tasks

- [ ] Add language dropdown to the navigation always
  - Branch: "dashboard/language"
  - Message: "[dashboard][feature] add language to all the navigation"

- [ ] Add page components props to all the pages (title, description and etch)
- [ ] Add all the pages, with only tests, mocked data
  - [ ] Make all the pages works like before, even with only mocks
- [ ] Add all te ts as translations
- [ ] All forms should be a sort of modals
- [ ] Have a config folder for:
  - [ ] Wiki form structure
  - [ ] selectOptions
  - [ ] user default settings
  - [ ] System options in general

## Profile

- [ ] Add translations for profile page, it and eng
  - Branch: "dashboard/profile_translations_page"
  - Message: "[dashboard][feature] add translations to the profile page"

- [ ] Add types for all the profile related functions
  - Branch: "dashboard/profile_types"
  - Message: "[dashboard][feature] add types to all the functions related to the profile feature"

- [ ] Test ui when data is empty, and when it is very long
  - Branch: "dashboard/profile_view_data"
  - Message: "[dashboard] Test ui when data is empty, and when it is very long"

- [ ] Add tests for all the functions involved with the profile page
  - Branch: "dashboard/profile_test"
  - Message: "[dashboard][feature] add tests for all functions related to the profile page"

- [ ] Add skeleton to each part of the app loading
  - Branch: "dashboard/profile/loader"
  - Message: "[dashboard][profile] Add skeleton to each part of the app loading"

- [ ] "Add the contributions section"
  - Branch: "dashboard/profile/contributions"
  - Message: "[dashboard][profile] Add the contributions section"

- [ ] Remvoe the share button, and add the possibility to see links of a user
  - Branch: "dashboard/profile/links"
  - Message: "[dashboard][profile] Remvoe the share button, and add the possibility to see links of a user"

- [ ] Add the logic for which the profile of the current user is different from the other users profile view
  - Branch: "dashboard/profile/user_view"
  - Message: "[dashboard][profile] Add the logic for which the profile of the current user is different from the other users profile view"

- [ ] Use the right sizes, so all the components are omogeneous
  - Branch: "dashboard/profile/component_size"
  - Message: "[dashboard][profile] Use the right sizes, so all the components are omogeneous"

- [ ] Add the avatar editor
  - Branch: "dashboard/profile/avatar"
  - Message: "[dashboard][profile] Add the avatar editor"

## Change password profile

## Settings profile

## Website

- [ ] Make the logo for Dance Diction
- [ ] Create a landing page for Dance Diction
  - [ ] Create the homepage
  - [ ] Create the "work with us" page
  - [ ] Create the "Our mission" page
  - [ ] Create the "person wiki" page
    - [ ] Create the "person wiki history contributions" page
    - [ ] Create the "person wiki history detail" page
    - [ ] Create the "person wiki history detail diff" page
